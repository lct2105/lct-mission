...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.TriggerCallback('lct-mission:server:CheckMission', function(data)
...
RegisterNetEvent("lct-mission:client:TakeDailyMission", function()
    ...
    if QBCore.Functions.GetPlayerData().metadata["dailymission"] == 0 or not QBCore.Functions.GetPlayerData().metadata["dailymission"] then  
        QBCore.Functions.TriggerCallback('lct-mission:server:CheckMission', function(data)
            if data then 
                ...
                TriggerServerEvent("lct-mission:server:TakeDailyMission", Random_Mission)
                TriggerEvent("lct-mission:client:CheckProgress", "dailymission")
            end
        end, "dailymission")
    else 
...
RegisterNetEvent("lct-mission:client:TakeHourlyMission", function()
    if QBCore.Functions.GetPlayerData().metadata["hourlymission"] == 0 or not QBCore.Functions.GetPlayerData().metadata["hourlymission"] then
        QBCore.Functions.TriggerCallback('lct-mission:server:CheckMission', function(data)
            if data then 
                ...
                TriggerServerEvent("lct-mission:server:TakeHourlyMission", Random_Mission)
                TriggerEvent("lct-mission:client:CheckProgress", "hourlymission")
            end
        end, "hourlymission")
    else 
        QBCore.Functions.Notify("Bạn đã nhận một nhiệm vụ rồi, bạn có thể kiểm tra tiến độ")
...
RegisterNetEvent("lct-mission:client:TakeHiddenMission", function(mission)
    ...
    if not QBCore.Functions.GetPlayerData().metadata[ID] then
        if not QBCore.Functions.GetPlayerData().metadata[ID.."_done"] then 
            ...
            TriggerServerEvent("lct-mission:server:TakeHiddenMission", ID)
            TriggerEvent("lct-mission:client:CheckHiddenProgress", mission)
        else 
...
RegisterNetEvent("lct-mission:client:CheckHiddenProgress", function(key)
    if Config.Hidden_Mission[key] then
        if QBCore.Functions.GetPlayerData().metadata[Config.Hidden_Mission[key].id] then
            TriggerServerEvent("lct-mission:server:CheckProgress", Config.Hidden_Mission[key].id, Config.Hidden_Mission[key].required, Config.Hidden_Mission[key].reward_item, Config.Hidden_Mission[key].reward_money)
        end
    end 
end)
